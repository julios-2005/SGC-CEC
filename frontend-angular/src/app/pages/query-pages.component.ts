...
import { Coordinador, Especialista, Planificacion } from '../core/models';
...
@Component({ changeDetection: ChangeDetectionStrategy.Eager, standalone: true, imports: [CommonModule, FormsModule, RouterLink], templateUrl: './templates/coordinator-query.component.html', styleUrl: './original-styles/coordinator-query.css' })
export class CoordinatorQueryComponent implements OnInit, OnDestroy {
  private readonly optionsRequest = new LatestRequest();
  private readonly plansRequest = new LatestRequest();
  loading = false;
  coordinators: Coordinador[] = []; plans: Planificacion[] = []; selectedId: number | null = null; selected?: Coordinador; error = '';
  constructor(readonly api: ApiService) {}
  ngOnInit(): void { this.optionsRequest.run(this.api.get<Coordinador[]>('coordinadores/activos'), { next: data => this.coordinators = data, error: e => this.error = this.api.errorMessage(e) }); }
  ngOnDestroy(): void { this.optionsRequest.cancel(); this.plansRequest.cancel(); }
  select(): void {
    this.plansRequest.cancel(); this.selected = this.coordinators.find(item => item.id === this.selectedId);
    this.plans = []; this.error = ''; this.loading = !!this.selectedId;
    if (!this.selectedId) return;
    this.plansRequest.run(this.api.get<Planificacion[]>(`planificaciones/coordinador/${this.selectedId}`), {
      next: data => { this.plans = data; this.loading = false; },
      error: e => { this.error = this.api.errorMessage(e); this.loading = false; },
    });
  }
  print(): void { if(this.selectedId) this.api.openFile(`coordinadores/${this.selectedId}/listado-pdf`); }
  modalityLabel(value:string):string { return ({PRESENCIAL:'Presencial',VIRTUAL:'Virtual',HIBRIDO:'Híbrido'} as Record<string,string>)[value] || value; }
}
